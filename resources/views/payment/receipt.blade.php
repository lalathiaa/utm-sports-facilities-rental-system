<x-app-layout>
    ...
    <div style="max-width:800px;margin:0 auto;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
            @auth
                @if(Auth::user()->isRentalOfficer())
                    <a href="{{ route('bookings.all') }}" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;color:var(--slate-400);text-decoration:none;font-weight:500;">
                ...
                @else
                    <a href="{{ route('bookings.my') }}" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;color:var(--slate-400);text-decoration:none;font-weight:500;">
                        <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                        Back to My Bookings
                    </a>
                @endif
        ...
        @if(session('success'))
            <div class="utm-alert utm-alert-success animate-in" style="margin-bottom:20px;">
        ...
        <div class="utm-card animate-in" style="overflow:hidden;">
            ...
            <div class="slip-header">
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;position:relative;z-index:1;">
                    <div style="display:flex;align-items:center;gap:14px;">
                        <div style="width:44px;height:44px;background:white;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            <svg viewBox="0 0 40 20" width="32" height="16">
                                <text x="50%" y="75%" dominant-baseline="middle" text-anchor="middle" font-family="Georgia,serif" font-weight="700" font-size="11" fill="#8B0000">UTM</text>
                            </svg>
                        </div>
                        <div>
                            <div style="font-size:16px;font-weight:700;color:white;">Payment Receipt</div>
                    ...
                    <div style="text-align:right;">
                        ...
                        <div style="font-size:20px;font-weight:800;color:white;font-family:'Plus Jakarta Sans',sans-serif;">#{{ str_pad($booking->id, 6, '0', STR_PAD_LEFT) }}</div>
                    </div>
                </div>
            </div>

            {{-- Status Strip --}}
            <div style="padding:12px 32px;background:rgba(5,150,105,.06);border-bottom:1px solid var(--slate-100);display:flex;align-items:center;justify-content:center;gap:10px;">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="color:#059669;">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span class="badge badge-confirmed" style="font-size:13px;padding:5px 14px;">Payment Successful · Booking Confirmed</span>
            </div>
            ...
            <div style="padding:28px 32px;display:flex;flex-direction:column;gap:24px;">
                ...
                {{-- Payment Info --}}
                @if($booking->payment)
                <div style="background:rgba(5,150,105,.04);border:1px solid rgba(5,150,105,.15);border-radius:var(--radius-md);padding:18px 20px;">
                    <div style="font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#059669;margin-bottom:12px;">Payment Information</div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                        <div>
                            <div style="font-size:10px;color:var(--slate-400);text-transform:uppercase;letter-spacing:.06em;">Transaction ID</div>
                            <div style="font-size:12px;font-weight:600;color:var(--slate-700);margin-top:3px;font-family:monospace;word-break:break-all;">{{ $booking->payment->transaction_id ?? 'N/A' }}</div>
                        </div>
                        <div>
                            <div style="font-size:10px;color:var(--slate-400);text-transform:uppercase;letter-spacing:.06em;">Payment Method</div>
                            <div style="font-size:13px;font-weight:600;color:var(--slate-700);margin-top:3px;text-transform:capitalize;">{{ $booking->payment->payment_method ?? 'Card' }}</div>
                        </div>
                        <div>
                            <div style="font-size:10px;color:var(--slate-400);text-transform:uppercase;letter-spacing:.06em;">Paid On</div>
                            <div style="font-size:13px;font-weight:600;color:var(--slate-700);margin-top:3px;">{{ $booking->payment->paid_at?->format('d M Y, h:i A') ?? 'N/A' }}</div>
                        </div>
                        <div>
                            <div style="font-size:10px;color:var(--slate-400);text-transform:uppercase;letter-spacing:.06em;">Amount Paid</div>
                            <div style="font-size:16px;font-weight:800;color:var(--utm-maroon);margin-top:3px;">RM {{ number_format($booking->payment->amount, 2) }}</div>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div>
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;">
                        @foreach($relatedBookings as $rb)
                            <span style="padding:6px 14px;background:rgba(139,0,0,.06);border:1px solid rgba(139,0,0,.12);border-radius:100px;font-size:13px;font-weight:600;color:var(--utm-maroon);">{{ $rb->slotLabel() }}</span>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:6px;">
                        @foreach($booking->equipment as $eq)
                            <div style="display:flex;justify-content:space-between;align-items:center;font-size:13.5px;">
                                ...
                                <span style="color:var(--slate-600);font-weight:600;">RM {{ number_format($eq->pivot->price_snapshot * $eq->pivot->quantity * $relatedBookings->count(), 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
